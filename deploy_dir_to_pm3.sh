...
echo "Build on pm3:"
ssh -J pm3@lab.os.itec.kit.edu pm3@pm3 "cd /unser_zeug/git/deploy_script/repo/power_management_linux && make -j20 LOCALVERSION=-$githash"
end_time=$(date +%s)
...
ssh -J pm3@lab.os.itec.kit.edu pm3@pm3 "bash -c 'cd /unser_zeug/git/deploy_script/repo/power_management_linux && sudo -S <<< \"pm3\" make modules_install -j20 LOCALVERSION=-$githash'"
echo ""
echo "Install kernel on pm3 (root password will be autotyped):"
ssh -J pm3@lab.os.itec.kit.edu pm3@pm3 "bash -c 'cd /unser_zeug/git/deploy_script/repo/power_management_linux && sudo -S <<< \"pm3\" make install -j20 LOCALVERSION=-$githash'"
